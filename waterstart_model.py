...
class CNN(nn.Module):
    def __init__(self, window_size: int, n_features: int, n_cur: int):
        self.window_size = window_size
        self.n_cur = n_cur
        self.conv1 = nn.Conv1d(3, 3, kernel_size=(1, 3))
        l_in = window_size - (3 - 1)
        l_out = 32
        self.conv2 = nn.Conv1d(3, l_out, kernel_size=(1, l_in - l_out + 1))
        self.conv3 = nn.Conv1d(l_out, n_features, kernel_size=(n_cur, l_out))

    def forward(self, x: torch.Tensor):
        assert x.size(-1) == self.window_size

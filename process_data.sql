...
SELECT
    rowid AS id,
    ROW_NUMBER() OVER (PARTITION BY timestamp, basecur_id, quotecur_id, type_id) AS rn
FROM rawdata
)

DELETE FROM rawdata
WHERE rowid IN (SELECT id FROM cte WHERE rn > 1);
...
WITH group_count(value) AS (
    SELECT COUNT(*)
    FROM (
        SELECT DISTINCT basecur_id, quotecur_id, type_id
        FROM processed_data
    )
)
...
) < (SELECT value FROM group_count LIMIT 1)
...
SELECT
    ...
    END)
FROM processed_data pd, types2 t2;
...
INNER JOIN long_format_processed_data pd2 ON
    pd1.timestamp = pd2.timestamp AND
    pd1.basecur_id = pd2.basecur_id AND
    pd1.quotecur_id = pd2.quotecur_id AND
    pd1.type2_id = pd2.type2_id
INNER JOIN currencies c ON pd1.basecur_id = c.id
WHERE c.value = 'eur'
GROUP BY pd1.timestamp, pd1.basecur_id, pd1.quotecur_id, pd1.type2_id, pd1.type_id;
...
LEFT JOIN euro_only_processed_data epd ON
    pd.timestamp = epd.timestamp AND
    pd.basecur_id = epd.quotecur_id AND
    pd.type2_id = epd.type2_id AND
    pd.type_id = epd.type_id;
